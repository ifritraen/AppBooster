...
- **Saved Target Repo**: ifritraen/AppBooster
...
- Inspect prompt to determine increment tier:
  - **Normal Bug Fix / Minor Edit**: **Do NOT bump version** (keep current version).
  - **Feature Update** (contains 'major', 'big', or explicit release bump): `+0.0.1` or `+0.1.0`.
  - **New Major Version** (contains 'new version' or 'v2'): `+1.0.0`.
- Update `pubspec.yaml` version line (`version: X.Y.Z+B`) or `build.gradle.kts` version variables (`major`, `minor`, `patch`) only when version bump is explicitly required.
